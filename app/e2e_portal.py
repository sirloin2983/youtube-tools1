...
def run_mounted_phase(browser, tmp, shots, check, events):
    ...
    try:
        ...
        for tid, verfrag in (("studio", "v0.6.0"), ("cut2resolve", None), ("transcribe", "v0.14.1")):
        ...
        tab = info.value
        tab.wait_for_load_state()
        check(wait_js(tab, "document.querySelector('#ver') && document.querySelector('#ver').textContent === 'v0.14.1'", 20000),
...
def run_child_process_phase(browser, tmp, shots, check, events):
    ...
    try:
        ...
        check(("ポート %d" % ports["transcribe"]) in meta and "v0.14.1" in meta and "入口に取り込み" not in meta,
