...
v0.2.0 では切り抜きスタジオを取り込みます(http://localhost:8700/studio/)。文字起こしと cut2resolve は、今は別のプログラムとして起動します。
...
■ 使い方
  ...
  3. 使うツールの「開く」を押すと、そのツールの画面が新しいタブで開きます
     (切り抜きスタジオは入口と同じアドレスの /studio/。ほかの2つはそれぞれのアドレス)
...
■ 入口に取り込んだツール(v0.2.0 では切り抜きスタジオ)
  入口のプログラムの中で動くので、カードに「入口に取り込み」と出て、「停止」「再起動」は押せません(「すべて終了」で一緒に終わります)。
  アクセスの記録は app\logs\studio.log に、スタジオ自身の記録は今までどおり clip-studio\studio.log に出ます。
...
■ テスト
  ...
  python -m unittest app/test_mount.py -v       (スタジオの取り込み: /studio/ の画面・API・CSP・合言葉・二重起動の防止など)
