...
def main():
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            check(navi() == 1 and pg.evaluate(active_body), "Shift+S で次の行(入力欄には入らない)")
            pg.keyboard.press("Shift+W")
            check(navi() == 0, "Shift+W で前の行")
            pg.keyboard.press("Shift+D")
            check(navi() == 1, "Shift+D で次の未校正へ(1行目は校正済みなのでとばす)")
            pg.keyboard.press("Shift+F")
            check(navi() == 4, "Shift+F で次の要確認へ(2行目 → 5行目)")
            pg.keyboard.press("Shift+F")
            check(navi() == 4 and "要確認" in pg.inner_text("#toast"), "Shift+F: これより後に要確認が無ければ移動せず、案内が出る")
            pg.keyboard.press("Shift+A")
            check(navi() == 3, "Shift+A で前の未校正へ")
            # タグ(聞き取れない・声が重なる・BGM)と話者
            pg.keyboard.press("Shift+X")
            check(pg.locator("#segs .seg").nth(3).evaluate("e => e.classList.contains('tagged')"), "Shift+X で「聞き取れない」")
            pg.keyboard.press("Shift+X")
            check(not pg.locator("#segs .seg").nth(3).evaluate("e => e.classList.contains('tagged')"), "もう一度で外れる")
            pg.keyboard.press("Shift+C")
            pg.keyboard.press("Shift+1")
            check(pg.locator("#segs .spk").nth(3).input_value() == "S1", "Shift+1 で話者を割り当て")
            # 入力欄の中では、Shift+文字は普通に入力できる(操作キーは働かない)
            pg.keyboard.press("Shift+T")
            check(pg.evaluate("document.activeElement === document.querySelectorAll('#segs textarea')[3]"), "Shift+T で行の文字を直す(入力欄へ)")
            ...
            pg.keyboard.press("Escape")
            # Shift+Space: 校正済みにして次へ(自動再生あり)
            pg.keyboard.press("Shift+W")
            pg.keyboard.press("Shift+B")
            check(pg.is_checked("#autoNext"), "Shift+B で「移動したら自動で再生」")
            ...
            pg.keyboard.press("Shift+Space")
            pg.keyboard.press("Shift+B")
            check(not pg.is_checked("#autoNext"), "Shift+B でオフ")
            # 自動再生
            pg.check("#autoNext")
            ...
            pg.click("#btnFocus")
            check(pg.is_hidden("aside"), "集中モード: 左のパネルが隠れる")
            pg.keyboard.press("Escape")
            pg.keyboard.press("Shift+G")
            check(pg.is_visible("aside"), "Shift+G で集中モードを終了")
            pg.wait_for_function("document.querySelector('#saveState').textContent.includes('保存しました')", timeout=8000)   # 再読み込みの前に、保存を待つ
            pg.goto("http://localhost:%d/%s" % (port, QS))
            pg.wait_for_selector("#txList .txi")
            check(pg.is_visible("aside") and pg.evaluate("document.documentElement.dataset.theme") == "dark" and pg.evaluate("getComputedStyle(document.documentElement).getPropertyValue('--fs').trim()") == "20px", "表示の設定(文字・色)は残り、集中モードは解除される")
            # 続きから
            items = pg.locator("#txList .txi")
            idx = [i for i in range(items.count()) if "大きい文書" not in items.nth(i).inner_text()][0]
            items.nth(idx).locator(".t").click()
            ...
            pg.keyboard.press("Escape")
            pg.keyboard.press("Shift+D")
            check(pg.evaluate("(() => { const i = [...document.querySelectorAll('#segs .seg')].findIndex(r => r.classList.contains('nav')); return i > 2001; })()"), "大きい文書でも Shift+D で次の未校正へ")
            pg.keyboard.press("Shift+Z")
            check(pg.locator("#segs .seg").count() == BIG - 1, "Shift+Z で行を削除")
