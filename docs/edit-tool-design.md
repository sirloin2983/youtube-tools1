...
- 対象 `redo_targets`: 「長い区間に文字が少ない」の印・校正済みでない・**機械の出力(`original`)がある文書では、同じ時刻の機械の出力と文字が同じ行だけ**(人・辞書が直した行は触らない。
  機械の出力の無い文書 = 文字起こしせずに開いた文書は見分けない)。範囲は行の前後 1 秒(`REDO_PAD`)だが、**隣の行・文書の範囲にはかからない**(隣の行を消さないため)。1回 30 行まで
- 認識: 今の範囲の再認識と同じ仕組み(`range_lines_real` → `finish_range_lines`)。モデルは文字起こしと同じ(kotoba なら `redoLarge` で large-v3)。
  **VAD は普通の強さ・短く区切る**(`redo_kwargs`: min_silence 250ms・speech_pad 200ms・chunk_length 10)。単語の時刻で分け、字幕の文字数も今の設定
- 良くなったか `redo_better`: 文字(記号・空白を除く)が増えた・まだ疑わしい印(文字が少ない・よくある誤認識・繰り返し・BGM)が付かない・avg_logprob が分かれば上がった(平均)。
  元の avg_logprob は、自動のとき文字起こしのジョブが覚えて渡す(`oldLp`)。手で押したときは分からないので比べない
- 置き換え `apply_redo`: 良くなった行だけ、**最後にまとめて1回で保存**(前の版は履歴 =「以前の版に戻す」)。途中で人が直した・校正した行は置き換えない。新しい行の id は `<元の id>-r1`…・話者は引き継ぐ。
  機械の出力(`original`)と単語の時刻(`words.json`)もその範囲を差し替える。文書に `redo = {model, rows, at}`
- **中止したら何も置き換えない**。**時間の上限 `REDO_MAX_SEC` = 600 秒**(超えたら残りの行はやめて、そこまでの結果で置き換える。ジョブの表示に書く)
- 設定: 新規の「終わったら、疑わしい所を自動で認識し直す」(`autoRedo`。**既定オフ**)・「kotoba のときは large-v3 を使う」(`redoLarge`。既定オン)。
  自動のときは文字起こしのジョブの最後に `redo` のジョブを足す(足せなければ文字起こしの注意に書く)。まとめて実行も同じ設定(`TX_KEYS`)。
  手で: 「文字をまとめて直す」の「疑わしい所を認識し直す」
- テスト: `test_metrics`(対象の選び方・良くなったか・設定)・`test_edit`(HTTP: 置き換え・校正済み・隣の行・履歴・対象なし)・`test_worker`(自動: 文字起こしのあとにジョブ・oldLp・置き換え・設定オフ)・`e2e_edit_tabs`(画面のボタン)。
  **本物の音声で効くかは未確認**(③-1 の 6 か所で試すと分かる。実機の確認のお願い)
...
- 入口 `app/autorun.py`: `start_docs(ids, overwrite)`(文書ごとに1つの実行 `Run(doc_id=…)`。段は `MODE_STEPS["doc"]` = 文字起こし・パック。
  ...
  元の動画が無ければ止めて理由を出す。受け口は入口の `POST /api/autorun/start-docs`(状態・中止は今までの `GET /api/autorun`・`POST /api/autorun/cancel`)
- 「編集」の履歴: 「選んで、まとめて実行」(`#txPick`)で行の左にチェック → 「まとめて実行」(`#txBatchGo`)・「パックがあれば作り直す」。進み具合は `#txRuns`(文書の実行だけ。2 秒おき・
