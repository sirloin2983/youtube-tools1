...
- **追加機能 ①〜⑦**(ユーザー 2026-09-26。`docs/edit-tool-design.md` の「12. 追加機能」。順番・決定・確認が要る所もそこ)。⑥(行から の端を広げる)・④(パックを最小限に・記録は cut2resolve の作業データ packs/)・①(Text+ の見た目 = けいふぉんと・黒・白ふち・黒ふち)・③-1(長い区間に文字が少ない行の印と集計 `tools/count_sparse_rows.py`)は実装済み。次は ② → ⑤(b) の確認 → ⑦ → ③-2(ユーザーの判断待ち)
