...
【これは何?】
  ...
  (YouTube からの動画・チャットの取得と、ランキング検索の YouTube Data API は使います)。
    ...
    入口(app)                          ツールをまとめて起動する。配信ごとの進み具合(案件)を見る・まとめて実行する
...
【起動と終了】
  起動: このフォルダの start-all.bat をダブルクリック(Mac は start-all.command)。
        ...
        スタジオは /studio/、編集は /transcribe/ で動きます(入口のカードの「開く」)。cut2resolve は部品なので、入口にカードを出しません。
...
【作業の流れ】
  ...
  2. 編集(スタジオの書き出しの結果の「編集で開く」か、入口の「編集」):
       1 文字起こし … 書き出した切り抜きを開いて文字起こし → 画面で聞きながら直す(校正)
         直した字幕は、スタジオの ③ でも書き出したマークの下に「セリフ」として、元の配信の時刻で見られます。
         履歴は配信ごとにまとまり、だれの・いつの・校正の進み具合が1行で見られます。
       2 カット … 残す区間をタイムライン(映像・波形・字幕)で決める。たたき台(行から・無音・時刻リスト・スタジオ)から始めて手で直せます。
         削った所を飛ばしてプレビューできます
       3 パック … 置き先の fps・縦横を選んで「パックを作る」。カット・字幕を直したら「作り直し」をお知らせします
  ...
  パックは、「編集」でカットを決めてあればそのとおりに、無ければ文字起こしの行だけを残して作ります。
  自動で作ったパックの字幕は校正前です。「編集」で直したら、3 パック のタブで作り直してください(「作り直し」の知らせが出ます)。
  「解析から全部」の解析は、スタジオの ② 解析の「解析の設定」で保存した値を使います(設定はスタジオのサーバーに保存されます)。

  重い処理(解析・書き出し・文字起こし・パック作り・音の波形)は、ツールを合わせて同時に2つまでです。
