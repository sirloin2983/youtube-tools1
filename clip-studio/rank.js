...
function paneHtml(){
  ...
  <section class="card cs-search" id="rkCond">
    ...
    <div class="cs-search-grid">
      ...
      <div class="fld cs-words"><label class="l" for="words">ワード</label><input type="text" id="words" placeholder="例: マイクラ 歌枠" maxlength="200" autocomplete="off"><span class="hint">空白かカンマで区切ります。空なら人気順だけ</span></div>
    </div>
    <div class="cs-opts">
      <label class="cs-opt"><span class="l">一致</span><select id="mode"><option value="any">どれか1つを含む</option><option value="all">すべて含む</option></select></label>
      <label class="cs-opt"><span class="l">探す場所</span><select id="inDesc"><option value="1">タイトル+概要欄</option><option value="0">タイトルだけ</option></select></label>
      <label class="cs-opt"><span class="l">1事務所の表示</span><select id="top"><option value="10">10本</option><option value="20" selected>20本</option><option value="50">50本</option><option value="100">100本</option></select></label>
    ...
    <div class="fld"><span class="l">対象の事務所 <span class="muted">(登録は「設定」の「事務所の登録」)</span></span><div class="chips" id="agChecks" role="group" aria-label="対象の事務所"></div></div>
  ...
  <div id="results"><div class="empty"><b>まだ検索していません</b>条件を決めて「検索する」を押すと、事務所ごとの人気配信がここに並びます</div></div>`;
}
function renderAgChecks(){
  ...
  const cur = [...box.querySelectorAll('.agc')];
  const sel = cur.length ? cur.filter(x => x.checked).map(x => x.value) : lsGet('ags');
  box.innerHTML = R.reg.agencies.map(a => { const n = a.channels.filter(c => c.status === 'ok').length;
    return `<label class="cs-chip"><input type="checkbox" class="agc" value="${esc(a.id)}"${(sel ? sel.includes(a.id) : n > 0) ? ' checked' : ''}>${esc(a.name)}<span class="cs-chip-n num">${n}</span></label>`; }).join('') || '<span class="hint">事務所がありません</span>';
}
...
async function startSearch(){
  ...
  setBusy(true); $('#results').innerHTML = skeleton(); R.result = null; R.picked.clear(); paintPick();
...
function renderResults(){
  ...
  html += `<p class="hint rk-sum">調べた動画: <span class="num">${fmtN(r.videos)}</span>本 ・ 使用ユニット: <span class="num">${fmtN(r.quota)}</span>(1日の目安 10,000) ・ 再生数は検索した時点の値です</p>`;
  html += r.agencies.map(a => `<section class="card rk-ag"><details class="ag" open><summary><span class="ag-name">${esc(a.name)}</span><span class="card-sub">該当 <span class="num">${fmtN(a.matched)}</span>本 ・ 期間内の配信 <span class="num">${fmtN(a.scanned)}</span>本 ・ <span class="num">${fmtN(a.channels)}</span>チャンネル</span></summary>
    ${a.channels === 0 ? '<p class="hint rk-none">解決済みのチャンネルがありません。設定の「事務所の登録」で追加してください。</p>' : !a.items.length ? '<p class="hint rk-none">条件に合う配信アーカイブがありませんでした。</p>' :
    `<div class="tbl-wrap"><table class="rk-table"><thead><tr><th class="ck"><span class="sr-only">選択</span></th><th class="rk">#</th><th class="hide-s"><span class="sr-only">サムネイル</span></th><th>タイトル</th><th class="n">再生数</th><th class="n hide-s">高評価</th><th class="hide-s">配信日時</th><th class="n hide-s">長さ</th></tr></thead><tbody>${a.items.map((v, i) => row(v, i)).join('')}</tbody></table></div>`}
...
function row(v, i){
  ...
  return `<tr data-vid="${esc(v.id)}"><td class="ck">${id ? `<input type="checkbox" class="pk" aria-label="解析に追加する: ${esc(v.title)}" data-id="${esc(v.id)}" data-title="${esc(v.title)}" data-channel="${esc(v.channel)}">` : ''}</td><td class="rk num">${i + 1}</td>
    ...
    <td class="tt">${id ? `<a href="${link}" target="_blank" rel="noopener noreferrer">${esc(v.title)}</a>` : esc(v.title)}<div class="hint">${esc(v.channel)}</div>
      <div class="row tt-act">${id ? `<button type="button" class="btn small soft add1" data-id="${esc(v.id)}" data-title="${esc(v.title)}" data-channel="${esc(v.channel)}">解析に追加</button>` : ''}<span class="chip"></span></div></td>
